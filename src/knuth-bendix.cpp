...
.. doctest::
  ...
  void init_runner(py::module& m) {
    py::class_<Runner, Reporter> runner(m, "Runner");
    runner.def("run", &Runner::run, runner_doc_strings::run)
        .def(
            "run_for",
            [](Runner& runner, std::chrono::nanoseconds t) {
              runner.run_for(t);
            },
            runner_doc_strings::run_for)
        .def("timed_out", &Runner::timed_out, runner_doc_strings::timed_out)
        .def(
            "run_until",
            [](Runner& runner, std::function<bool()>& f) {
              runner.run_until(f);
            },
        ...
        .def("finished", &Runner::finished, runner_doc_strings::finished)
        .def("started", &Runner::started, runner_doc_strings::started)
        .def("running", &Runner::running, runner_doc_strings::running)
        .def("kill", &Runner::kill, runner_doc_strings::kill)
        .def("dead", &Runner::dead, runner_doc_strings::dead)
        .def("stopped", &Runner::stopped, runner_doc_strings::stopped)
